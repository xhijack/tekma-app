  ...
  get_item_support(frm) {
    frappe.call({
      ...
      callback: function (r) {
        ...
        const missing_warehouse = items
          .filter((i) => !i.s_warehouse)
          .map((i) => i.item);
        if (missing_warehouse.length) {
          frappe.msgprint({
            title: __("Default Warehouse Tidak Ditemukan"),
            message: __(
              "Item berikut tidak memiliki default warehouse untuk company ini, kolom Source Warehouse akan kosong:<br><b>{0}</b>",
              [missing_warehouse.join("<br>")],
            ),
            indicator: "orange",
          });
        }
...
async function update_ratio_valuation_rate(frm) {
