...
export default class Grid {
	...
	make_head() {
		...
		const $heading = $(this.parent).find(
			".grid-heading-row"
		);

		if (this.is_mobile_card_grid()) {
			this.header_row = null;
			this.header_search = null;
		...
		if (
			...
			this.header_search.show_search_row()
		) {
	...
	refresh() {
		...
		//custom
		if (this.is_mobile_card_grid()) {
			this.render_mobile_empty_state();
		} else {
			this.wrapper
				.find(".grid-empty")
				.toggleClass(
	...
	render_result_rows($rows) {
		...
		const GridRowClass = this.get_grid_row_class();
	...
	is_mobile_card_grid() {
		return (
			is_mobile_device() &&
			["Table", "Table MultiSelect"].includes(
				this.df?.fieldtype
			)
		);
	}
	...
	get_grid_row_class() {
		return this.is_mobile_card_grid()
			? MobileGridRow
			: GridRow;
	}

	render_mobile_empty_state() {
		if (!this.is_mobile_card_grid()) {
			return;
		}
